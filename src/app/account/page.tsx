			...
			<div className="bg-surface border-b border-border pt-12 pb-8">
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					<div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
						...
						<Button
							variant="outline"
							className="flex items-center gap-2 bg-white hidden md:flex"
						>
				...
				<div className="flex flex-col lg:flex-row gap-8">
					...
					<div className="w-full lg:w-64 shrink-0 space-y-2">
						...
						>
							...
							<span
								className={`text-xs px-2 py-0.5 rounded-full ${activeTab === "orders" ? "bg-white/20 text-white" : "bg-primary/10 text-primary"}`}
							>
								{mockOrders.length}
							</span>
						</button>
								...
								<div>
									...
									<div className="space-y-4">
										{mockOrders.slice(0, 2).map((order) => (
											<Card
												key={order.id}
												className="p-0 overflow-hidden group hover:shadow-md transition-shadow border-border"
											>
													...
													<div className="flex items-start gap-4">
														...
														<div>
															<div className="flex items-center gap-3 mb-1">
																...
																<span
																	className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${order.statusColor}`}
																>
																	{order.status}
																</span>
															</div>
							...
							<div className="space-y-6 animate-fade-in-up">
								...
								<div className="space-y-4">
									{mockOrders.map((order) => (
										<Card
											key={order.id}
											className="overflow-hidden border-border transition-all hover:border-primary/30 hover:shadow-md"
										>
														...
														<div className="flex items-center gap-3 mb-3">
															...
															{order.status === "In Transit" && (
																<span className="text-sm font-medium text-text-secondary flex items-center gap-1">
																	<Truck className="w-4 h-4" /> Arriving by
																	April 5
																</span>
															)}
														</div>
								...
								<div className="grid sm:grid-cols-2 gap-6">
									<Card className="p-6 border-primary bg-primary/5 relative">
										<div className="absolute top-4 right-4">
											<span className="px-2.5 py-0.5 bg-primary rounded-full text-white text-xs font-bold">
												Default
											</span>
										</div>
										...
										<div className="flex gap-3">
											...
											<Button
												variant="outline"
												size="sm"
												className="text-error hover:bg-error/10 border-border"
											>
												Delete
											</Button>
										</div>
									...
									<button
										type="button"
										className="p-6 border-2 border-dashed border-border rounded-2xl flex flex-col items-center justify-center text-text-tertiary hover:text-primary hover:border-primary hover:bg-primary/5 transition-all min-h-[250px]"
									>
