				...
				<div className="w-full md:w-1/2 p-6 md:p-8 overflow-y-auto max-h-[50vh] md:max-h-none flex flex-col">
					<div className="flex flex-wrap gap-2 mb-3">
						{product.tags.slice(0, 2).map((tag) => (
							<span key={tag} className="px-2.5 py-0.5 rounded-full bg-primary/10 text-primary text-xs font-bold uppercase">
								{tag}
							</span>
						))}
					</div>
					...
					<div className="py-6 flex-1 text-text-secondary text-sm leading-relaxed line-clamp-4">
						{product.description || "Premium quality product sourced directly from natural environments. Prepared perfectly for the best outcome."}
					</div>
